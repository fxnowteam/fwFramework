...
class UploadHandler
{
    ...
    private function trim_file_name($name, $type) {
    	$name = hash('sha512',$name."".date("YmdHis"));
        ...
        if (!$error && $file->name) {
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
                     ...
                     *
                     */
	             $nomedoarquivo = str_replace("/home/".$_SESSION["servidor"]."/public_html/s/upc/up/files/","",$file_path);
                     @mysql_query("INSERT INTO at_imagens_".$_SESSION["cliente"]." (idagendamento, fotourl) VALUES ('".$_SESSION["idag_uploadimg"]."', '$nomedoarquivo')") or die(mysql_error());
